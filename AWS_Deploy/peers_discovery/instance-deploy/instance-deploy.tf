...
variable "demo_east_ami" {
    type = string
    default = "ami-02cfbd070b4ad04cb2"
}
...
resource "aws_instance" "demo_server" {
    ...
    tags = {
      "Name" = "gncfd_demo_discoverer",
    }

    user_data = <<-EOF
      #!/bin/bash
      echo "export DISCOVERER_PORT=${var.demo_discoverer_port}" >> /home/ec2-user/.bashrc
      sudo systemctl start discoverer_discovery.service
    EOF
}
...
resource "aws_instance" "demo_client_east" {
    ...
    user_data = <<-EOF
      ...
      echo "export DISCOVERER_PORT=${var.demo_discoverer_port}" >> /home/ec2-user/.bashrc
      sudo systemctl start client_discovery.service
      EOF
}
